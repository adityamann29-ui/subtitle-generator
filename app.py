...
# only confirmed working models on IBM Watson
LANGUAGES = {
    'en-US': 'English (US)',
    'en-GB': 'English (UK)',
    'fr-FR': 'French',
...
try:
    if IBM_API_KEY and IBM_URL:
        ...
        print(f"IBM Watson connected! models available: {len(models['models'])}")
    else:
        print("ERROR: IBM credentials missing!")
        print(f"IBM_API_KEY: {IBM_API_KEY}")
...
def extract_audio(video_file_path):
    try:
        ...
        print(f"audio extracted! size: {os.path.getsize(audio_file_path) / (1024*1024):.2f} MB")
...
def get_subtitles_from_ibm(audio_path, lang_code='en-US'):
    try:
        ...
        print(f"response received! results: {len(ibm_response.get('results', []))}")
...
def create_srt_file(segments, srt_output_path):
    try:
        ...
        for idx, seg in enumerate(segments):
            ...
            srt_file.write(convert_to_srt_time(seg['start']) + ' --> ' + convert_to_srt_time(seg['end']) + '\n')
            srt_file.write(seg['text'] + '\n')
            srt_file.write('\n')
        srt_file.close()
        print(f"SRT created: {srt_output_path}")
        return True
    except Exception as e:
        print(f"SRT error: {e}")
        return False
...
def upload_video():
    ...
    try:
        # check IBM Watson
        if ibm_stt is None:
            return jsonify({'error': 'IBM Watson not connected. Check API credentials in Railway Variables.'}), 500
        ...
        if audio_save_path is None:
            delete_temp_files(video_save_path, '')
        ...
        if subtitle_segments is None:
            ...
            return jsonify({'error': 'IBM Watson could not process audio. Check logs for details.'}), 500
